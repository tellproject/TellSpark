...
class Q5 extends ChQuery {
  ...
  override def execute(st: String, cm: String, cn:Int, cs:Int, mUrl:String): Unit = {
    ...
    val orderRdd = new TRDD[TRecord](scc, "order", new ScanQuery(), ChTSchema.orderSch).filter(r => {
      r.getValue("O_ENTR_D").asInstanceOf[Long] >= 20070102
    }).map(r => {
      // key(o_c_id, o_w_id, o_d_id) val(o_id)
      val key = (r.getValue("O_C_ID").asInstanceOf[Int], r.getValue("O_W_ID").asInstanceOf[Int], r.getValue("O_D_ID").asInstanceOf[Int])
      (key, r.getValue("O_ID").asInstanceOf[Int])
    })
    ...
    val customerRdd = new TRDD[TRecord](scc, "customer", new ScanQuery(), ChTSchema.customerSch).map(r => {
      ...
      val key = (r.getValue("C_ID").asInstanceOf[Int], r.getValue("C_W_ID").asInstanceOf[Int], r.getValue("C_D_ID").asInstanceOf[Int])
      (key, r.getValue("C_STATE").asInstanceOf[String])
    })
    ...
    val orderline2 = orderlineRdd.map(r => {
      ...
      val key =(r.getValue("OL_W_ID").asInstanceOf[Int],r.getValue("OL_I_ID").asInstanceOf[Int])
      (key, r.getValue("OL_AMOUNT").asInstanceOf[Double])
    })
    val orderline2_stock = orderline2.join(stockRdd.map(r => {
      val key = (r.getValue("S_W_ID").asInstanceOf[Int], r.getValue("S_I_ID").asInstanceOf[Int])
      (key, key)
    }))
    //ol_w_id = s_w_id and ol_i_id = s_i_id
    ...
    val c_o_ol_s_su_s = c_o_ol_s.map(r => {
      ...
      .join(supplierRdd.map(r => { (r.getValue("SU_NATIONKEY").asInstanceOf[String], r.getValue("SU_SUPPKEY").asInstanceOf[Int])}))
      .map(r => {
      ...
      //mod((s_w_id * s_i_id),10000)
        ...
        val key = (r.getValue("s_w_id").asInstanceOf[Int] *r.getValue("s_i_id").asInstanceOf[Int]) % 10000
        (key, key)
      }))
    //su_nationkey = n_nationkey
    val partial_res = c_o_ol_s_su_s.map(r => (r._2._1._1, r._2._1._2))
      .join(nationRdd.map(r => (r.getValue("n_nationkey").asInstanceOf[String], (r.getValue("n_regionkey").asInstanceOf[Int], r.getValue("n_name").asInstanceOf[String]))))
      .map(r => {(r._2._2._1, (r._2._1, r._2._2._2))})
      .join(regionRdd.map(r => (r.getValue("r_regionkey").asInstanceOf[Int], r.getValue("r_regionkey").asInstanceOf[Int])))
