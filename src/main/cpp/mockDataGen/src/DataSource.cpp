...
string DataSource::tpchSentence(){
	if(randomTrue(1/5))
		...
		tpchVerbPhrase() + " " + tpchNounPhrase() + " " + tpchTerminators[randomUniformInt(0,tpchTerminators.size()-1)];
	else
...
string DataSource::tpchNounPhrase(){
	if(randomTrue(1/4))
		...
		return string(tpchAdjectives[randomUniformInt(0,tpchAdjectives.size()-1)])  + ", " +
		tpchAdjectives[randomUniformInt(0,tpchAdjectives.size()-1)]  + " " + tpchNouns[randomUniformInt(0,tpchNouns.size()-1)];
	else
...
string DataSource::tpchVerbPhrase(){
	if(randomTrue(1/4))
		...
		return string(tpchVerbs[randomUniformInt(0,tpchVerbs.size()-1)]) + " " + tpchAdverbs[randomUniformInt(0,tpchAdverbs.size()-1)];
	else
		return string(tpchAuxiliaries[randomUniformInt(0,tpchAuxiliaries.size()-1)])  + " " +
		tpchVerbs[randomUniformInt(0,tpchVerbs.size()-1)] + " " + tpchAdverbs[randomUniformInt(0,tpchAdverbs.size()-1)];
}
...
string DataSource::genAlphanumeric64(int length) {
	string s = "";
	int rand;
	for(int i=0; i<length; i++){
		rand=0;
...
string DataSource::genAlphanumeric64(int minLength, int maxLength) {
	...
	oss << "ORIGINAL";
	oss << genAlphanumeric64(rLength-8-rPosition);
	return oss.str();
}
...
void DataSource::addTextStringCustomer(int minLength, int maxLength, const char* action, ofstream& stream, bool delimiter){
	int rLength=0;
	randomUniformInt(minLength,maxLength,rLength);
	int l1=0;
